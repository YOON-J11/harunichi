...
public class MemberControllerImpl implements MemberController {
	...
	@RequestMapping(value = { "/loginpage.do", "/addMemberForm.do", "/emailAuthForm.do",
			"/profileImgAndMyLikeSetting.do", "updateMyInfoForm.do" }, method = RequestMethod.GET)
	@Override
	...
	public ModelAndView kakaoCallback(@RequestParam("code") String code,
		...
		ResponseEntity<Map> tokenResponse = restTemplate.exchange(tokenUrl, HttpMethod.POST, tokenRequestEntity,
				Map.class);
		...
		ResponseEntity<Map> userInfoResponse = restTemplate.exchange(userInfoUrl, HttpMethod.GET, userInfoRequestEntity,
				Map.class);
		...
		if (userinfo.containsKey("kakao_account")) {
			...
			if (kakaoAccount.containsKey("birthday") && kakaoAccount.containsKey("birthyear")) {
				try {
					...
					cal.set(Integer.parseInt(birthyear), Integer.parseInt(birthday.substring(0, 2)) - 1,
		...
		if ("login".equals(mode)) {
			if (dbMember != null && dbMember.getId() != null) {
				...
				if (isEmailDuplicate) {
					try {
						HttpServletResponse response = ((org.springframework.web.context.request.ServletRequestAttributes) org.springframework.web.context.request.RequestContextHolder
	...
	public ModelAndView naverCallback(@RequestParam("code") String code,
		...
		String tokenUrl = "https://nid.naver.com/oauth2.0/token?grant_type=authorization_code" + "&client_id="
				+ clientId + "&client_secret=" + clientSecret + "&code=" + code + "&state=" + mode;
		...
		ResponseEntity<Map> userInfoResponse = restTemplate.exchange("https://openapi.naver.com/v1/nid/me",
				HttpMethod.GET, userInfoRequest, Map.class);
	...
	public String addMemberWriteForm(HttpSession session, HttpServletResponse response, HttpServletRequest request)
			throws IOException {
		MemberVo memberVo = (MemberVo) session.getAttribute("memberVo");
		response.setContentType("text/html; charset=UTF-8");

		if (memberVo == null || memberVo.getEmail() == null) {
			response.getWriter().write("<script>alert('비정상적인 접근입니다.'); location.href='" + request.getContextPath()
					+ "/member/addMemberForm.do';</script>");
			return null;
		}
		if (memberService.isEmailDuplicate(memberVo.getEmail())) {
	...
	@ResponseBody
	@Override
	public String addMemberProcess(@RequestParam("id") String id, @RequestParam("pass") String pass,
			@RequestParam("name") String name, @RequestParam("nick") String nick,
			@RequestParam("year") String yearString, @RequestParam(value = "gender", required = false) String gender,
			@RequestParam(value = "tel", required = false) String tel,
			@RequestParam(value = "address", required = false) String address, HttpSession session) throws Exception {
	...
	@ResponseBody
	@Override
	public ResponseEntity<Map<String, Boolean>> checkId(@RequestParam("id") String id, HttpServletRequest request,
	...
	public void profileImgAndMyLikeSettingProcess(
			@RequestParam("profileImg") org.springframework.web.multipart.MultipartFile profileImg,
			@RequestParam(value = "myLike", required = false) String[] myLikes, HttpServletRequest request,
		...
		try (PrintWriter out = response.getWriter()) {
			...
			if (memberVo == null) {
				out.println("<script>alert('잘못된 접근입니다. 다시 시도해주세요.'); location.href='" + request.getContextPath()
						+ "/member/addMemberForm.do';</script>");
				out.flush();
				return;
			}
			...
			try {
				...
				out.println("<script>alert('회원가입 처리 중 오류가 발생했습니다.'); location.href='" + request.getContextPath()
						+ "/member/addMemberForm.do';</script>");
				out.flush();
				return;
			}
			...
			out.println("<script>alert('회원가입이 완료되었습니다. 환영합니다!'); location.href='" + request.getContextPath()
					+ "/';</script>");
			out.flush();
		} catch (IOException e) {
			logger.error("응답 쓰기 오류", e);
	...
	public String updateMyInfoProcess(@RequestParam("id") String id,
			@RequestParam(value = "pass", required = false) String pass, @RequestParam("name") String name,
			...
			@RequestParam(value = "resetProfile", required = false) String resetProfile, HttpSession session,
		...
		try {
			...
			response.setContentType("text/html;charset=UTF-8");
			PrintWriter out = response.getWriter();
			out.println("<script>alert('회원정보 수정 중 오류가 발생했습니다.'); location.href='" + request.getContextPath()
					+ "/';</script>");
			out.flush();
			return null;
		}
	...
	@RequestMapping(value = { "/myBoardList.do", "/myBoardList" }, method = RequestMethod.GET)
	public String myBoardList(@RequestParam(value = "id", required = false) String targetId, HttpSession session,
			Model model) {

		MemberVo sessionMember = (MemberVo) session.getAttribute("member");
		String who = (targetId != null && !targetId.isEmpty()) ? targetId
				: (sessionMember != null ? sessionMember.getId() : null);

		if (who == null)
			return "member/loginpage";

		List<BoardVo> myBoards = memberService.getMyBoards(who);
		model.addAttribute("boardList", myBoards);

		Map<Integer, Boolean> likedPosts = new HashMap<>();
		if (sessionMember != null) {
			for (Integer bId : memberService.getLikedBoardIds(sessionMember.getId())) {
				likedPosts.put(bId, true);
			}
		}
	...
	@RequestMapping(value = { "/myLikeBoardList.do", "/myLikeBoardList" }, method = RequestMethod.GET)
	public String myLikeBoardList(@RequestParam(value = "id", required = false) String targetId, HttpSession session,
			Model model) {
		...
		List<BoardVo> likedBoards = memberService.getMyLikedBoards(who);
