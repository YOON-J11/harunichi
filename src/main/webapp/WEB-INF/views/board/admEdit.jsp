	...
	<div class="container">
		...
		<form action="${contextPath}/board/admin/updateAdmin" method="post">
			<table>
				<tr>
					<th>번호</th>
					<td>${board.boardId} <input type="hidden" name="boardId"
						value="${board.boardId}" />
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td><select name="boardCate">
							...
							<option value="생활정보"
								${board.boardCate == '생활정보' ? 'selected' : ''}>생활정보</option>
							<!-- 필요한 다른 카테고리 추가 -->
					...
					<td>
						<div class="image-preview">
							...
							<c:if test="${not empty board.boardImg1}">
								...
								<c:choose>
									<c:when
										test="${fn:startsWith(board.boardImg1,'http://') or fn:startsWith(board.boardImg1,'https://')}">
									...
									<c:otherwise>
										<img
											src="${contextPath}/resources/images/board/${board.boardImg1}"
											alt="이미지1"
											style="width: 120px; height: 120px; object-fit: cover"
											onerror="this.onerror=null;this.src='${contextPath}/resources/icon/image_placeholder.png'" />
									</c:otherwise>
							...
							<c:if test="${not empty board.boardImg2}">
								...
								<c:choose>
									<c:when
										test="${fn:startsWith(board.boardImg2,'http://') or fn:startsWith(board.boardImg2,'https://')}">
									...
									<c:otherwise>
										<img
											src="${contextPath}/resources/images/board/${board.boardImg2}"
											alt="이미지2"
											style="width: 120px; height: 120px; object-fit: cover"
											onerror="this.onerror=null;this.src='${contextPath}/resources/icon/image_placeholder.png'" />
									</c:otherwise>
							...
							<c:if test="${not empty board.boardImg3}">
								...
								<c:choose>
									<c:when
										test="${fn:startsWith(board.boardImg3,'http://') or fn:startsWith(board.boardImg3,'https://')}">
									...
									<c:otherwise>
										<img
											src="${contextPath}/resources/images/board/${board.boardImg3}"
											alt="이미지3"
											style="width: 120px; height: 120px; object-fit: cover"
											onerror="this.onerror=null;this.src='${contextPath}/resources/icon/image_placeholder.png'" />
									</c:otherwise>
							...
							<c:if test="${not empty board.boardImg4}">
								...
								<c:choose>
									<c:when
										test="${fn:startsWith(board.boardImg4,'http://') or fn:startsWith(board.boardImg4,'https://')}">
									...
									<c:otherwise>
										<img
											src="${contextPath}/resources/images/board/${board.boardImg4}"
											alt="이미지4"
											style="width: 120px; height: 120px; object-fit: cover"
											onerror="this.onerror=null;this.src='${contextPath}/resources/icon/image_placeholder.png'" />
									</c:otherwise>
			...
			<div class="button-group">
				<button type="submit" class="save"
					onclick="return confirm('게시글을 수정하시겠습니까?');">저장</button>
				<button type="button" class="cancel"
					onclick="location.href='${contextPath}/board/admin';">취소</button>
			</div>
		</form>
	</div>
</body>
