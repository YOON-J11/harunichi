        ...
        <c:forEach var="member" items="${memberList}">
          <li>
            <div class="profile-con">
              <a href="${contextPath}/mypage?id=${member.id}">
                <c:choose>
                  <c:when test="${not empty member.profileImg}">
                    <c:choose>
                      <c:when test="${fn:startsWith(member.profileImg, 'http')}">
                        <img class="profile-img" src="${member.profileImg}" alt="profile">
                      </c:when>
                      <c:otherwise>
                        <img class="profile-img" src="${contextPath}/resources/images/profile/${member.profileImg}" alt="profile">
                      </c:otherwise>
                    </c:choose>
                  </c:when>
                  <c:otherwise>
                    <img class="profile-img" src="${contextPath}/resources/icon/basic_profile.jpg" alt="basic">
                  </c:otherwise>
                </c:choose>
              </a>

              <p class="nick"><c:out value='${member.nick}'/></p>
              <p><span style="color:#a3daff; font-weight:bold;">LIKE </span><c:out value='${member.myLike}'/></p>
